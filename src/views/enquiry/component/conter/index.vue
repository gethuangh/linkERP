  ...
  <div>
    ...
    <div class="cont-table">
      ...
      >
        ...
        <el-table-column prop="courseValue" label="进程" sortable width="70" :resizable="false"></el-table-column>
        <el-table-column
          prop="result"
          align="center"
          label="结果"
        ...
        >
          <template scope="scope">
            <span style="color:red;text-align:center;">{{ scope.row.result }}</span>
          </template>
        </el-table-column>
        <el-table-column prop="number" label="询价单号" :resizable="false" sortable width="120">
          <template scope="scope">
            <span style="color:blue;">{{ scope.row.number }}</span>
          </template>
        </el-table-column>
        <el-table-column prop="priority" :resizable="false" label="优先级" sortable width="80">
          <template scope="scope">
            <span style="color:red; text-align:center;">{{ scope.row.priority }}</span>
          </template>
        </el-table-column>
              ...
              <div>
                <i>
                  <el-tooltip class="item" effect="dark" content="复制" placement="bottom">
                    ...
                    <i class="icon copy iconfont" title="复制" @click.stop="copyCode(scope)">&#xe603;</i>
                  </el-tooltip>
                </i>
                <i>
                  <el-tooltip class="item" effect="dark" content="渠道" placement="bottom">
                    ...
                    <i class="icon leave iconfont" @click.stop="getDitch(scope)">&#xe6dd;</i>
                  </el-tooltip>
                </i>
              </div>
            </div>
        ...
        <el-table-column prop="enquiryNum" :resizable="false" label="询价数量" sortable width="80">
          <template scope="scope">
            <span style="text-align:right;">{{ scope.row.enquiryNum }}</span>
          </template>
        </el-table-column>
        <el-table-column
        ...
        <el-table-column sortable label="客户接受价" width="140" header-align="center">
          <template slot-scope="scope">
            <div class="scope-flex">
              ...
              <i style="font-style:italic">{{scope.row.currency}}</i>
            </div>
          </template>
        </el-table-column>
        <el-table-column prop="date" label="税率" :resizable="false" sortable width="60">
          <template slot-scope="scope">
            <i style="font-style:italic;text-align:right">{{scope.row.taxRate}}</i>
          </template>
        </el-table-column>
        <el-table-column prop="date" :resizable="false" label="销售备注" sortable>
          <template scope="scope">
            <span style="color:red;">{{ scope.row.salesmanRemarks }}</span>
          </template>
        </el-table-column>
      </el-table>
      <!-- 弹窗表格 -->
...
<script>
import xiala from "../pull/index";
import tan from "../pop/index";
export default {
  data() {
    return {
      detail: {
  ...
  props: ["tableData", "show", "ditch", "result"],
  components: {
    tan,
    xiala
  },
  methods: {
    ...
    xiala(row, event, column) {
      this.table = true;
      console.log(row);
      this.$emit("offer", row.id, row.clientModel);
    ...
    getDitch(scope) {
      ...
      this.$emit("ditch", scope.row.clientModel);
    },
    all() {
      this.$emit("all");
    }
